...
\begin{flushleft}
	{\fontsize{14pt}{12pt}\selectfont
		\textbf{Streszczenie}}\\
	\vspace{1cm}
\tab Celem niniejszej pracy było \change{opracowanie}{stworzenie} aplikacji internetowej, która \change{pozwoliłaby}{pozwala} na wyszukiwanie połączeń lotniczych korzystając z danych zawartych na stronach internetowych przewoźników bądź z innych centr danych. Aplikacja została podzielona na część kliencką oraz serwerową. Klient został napisany przy użyciu technologii Angular 6, natomiast \change{część serwerowa}{serwer} w technologii Java 10. W pracy znajduje się opis architektury stworzonej aplikacji, modułu wyszukiwania połączeń lotniczych a także zagadnień teoretycznych związanych z projektowaniem interfejsu użytkownika dla przeglądarki internetowej.
\end{flushleft}
\vspace{0.5cm}
...
\begin{flushleft}
	{\fontsize{14pt}{12pt}\selectfont
		\textbf{Summary}}\\
	\vspace{1cm}
\tab The purpose of \add{the} thesis was to build a web application, which will be able to search flight connections using data \change{included on air}{from} websites \add{of airlines} or other data sources. \change{A}{The a}pplication \change{was divided}{is splat} into two parts: \add{a} client and \add{a} server. \change{C}{The c}lient was implemented using technology of Angular 6, whereas server in Java 10 technology. Description of architecture built application, module of air connections searching and theoretical issues related to building user interface for web application are included in this thesis. 
\end{flushleft}
\vspace{0.5cm}
...
Branża lotnicza to jedna z głównych gałęzi \change{dzisiejszego}{współczesnego} transportu. Za jej początek uznaje się pierwszy pomyślny lot braci Wright 17 grudnia 1903 roku na polach Kitty Hawk. To wydarzenie zapoczątkowało proces tworzenia się przemysłu lotniczego. W dzisiejszych czasach transport lotniczy uznaje się za najszybszy i najbardziej bezpieczny. Wykorzystuje się go między innymi w transporcie osób, towarów lub też w celach militarnych. Warto wspomnieć też o jego roli \change{jako prekursora}{w~rozwoju} lotów kosmicznych. Dzięki niemu powstała nieosiągalna wcześniej możliwość podróżowania po całym świecie w \change{możliwe}{względnie} \change{najkrótszym}{krótkim} czasie.\\
\hint{Akapit w~\LaTeX{}u można uzyskać wstawiając pusty wiersz.} 

W\add{~ramach} niniejszej pracy \change{podjęto się stworzenia aplikacji internetowej umożliwiającej}{stworzono aplikację internetową umożliwiającą} wyszukiwanie realnych połączeń lotniczych. Aplikacja ta składa się z części klienckiej oraz serwerowej. \change{Dla}{Celem} zwiększenia jej wydajności \change{do architektury została dodana relacyjna baza danych oraz mechanizmy cachowania danych}{zastosowano mechanizmy pamięci podręcznej}.
Jej \change{główną funkcjonalnością}{głównym zadaniem} jest zbieranie danych o połączeniach lotniczych z zewnętrznych serwisów oraz zasobów internetowych.
 Pozwala ona na uzyskanie informacji o lotach \change{uwzględniając dane}{z~uwzględnieniem danych} o ich cenie, \change{linii lotniczej go obsługującej}{liniach lotniczych je obsługujących}, \change{wymiarów i wagi}{wymiarach i~wagach} dozwolonego bagażu, \change{czasu}{czasie} podróży \change{czy}{lub} też \change{liczby}{liczbie} przesiadek.\\

Praca została podzielona na 6 rozdziałów. Rozdział pierwszy przedstawia zagadnienie wyszukiwania połączeń lotniczych wraz z opisem najważniejszych problemów\change{ które podjęto rozwiązaniu podczas powstawania}{, które starano się rozwiązać w~opracowywanej} aplikacji. Drugi rozdział opisuje \delete{obecnie} istniejące rozwiązania \add{w~dziedzinie wyszukiwania połączeń lotniczych, które dostępne są} na rynku. Jest to \change{opis}{zestawienie} dwóch komercyjnych aplikacji które zyskały duże uznanie swoich użytkowników. \change{Zakończony został}{Rozdział został zakończony} podsumowującym porównaniem obu aplikacji. Rozdział trzeci przedstawia projekt aplikacji. Znajdują się w nim schematy z opisem, które przestawiają architekturę powstałej aplikacji. \change{Obrazuje}{Obrazują one} podział funkcjonalności między \change{bazą}{bazę} danych, \change{warstwą}{warstwę} logiki biznesowej \change{czy też warstwą}{oraz warstwę} prezentacji. Opisane zostały również ścieżki komunikacji pomiędzy tymi warstwami.
Rozdział czwarty \change{przedkłada}{objaśnia} implementację aplikacji. W rozdziale tym zostaną przedstawione najważniejsze fragmenty oprogramowania tworzące funkcjonalność aplikacji. \change{Można będzie w nim znaleźć}{Znajdują się w~nim} informacje o użytych technologiach jak i zewnętrznych bibliotekach którymi się posłużono. Piąty rozdział poświęcony jest testom oprogramowania, które potwierdzają \change{prawidłowe}{zgodne z~wymaganiami} działanie aplikacji. Ostatni rozdział przedstawia uwagi i wnioski \change{odnoszące się do}{dotyczące} tematu pracy oraz stworzonej aplikacji.
\hint{Polecenie \texttt{chapter} automatycznie wstawia nową stronę.}
%\newpage

\chapter{Opis zagadnienia}
\label{chp:first}
\hint{Zmieniłem tytuł.}
Głównym zagadnieniem podjętym w \change{pracy było}{jest}  pozyskanie realnych danych o połączeniach lotniczych \change{które można by było zaprezentować w}{ i~zaprezentowanie ich w~}kompleksowym interfejsie oraz we względnie \change{optymalnym}{krótkim} czasie dla użytkownika powstałej aplikacji.

Zagadnienie to można podzielić na 3 części:
\begin{itemize}[noitemsep,topsep=0pt]
\item \change{Z}{z}nalezienie źródeł danych o połączeniach lotniczych\add{,}
\item \change{P}{p}arsowanie różnych \change{rodzajów}{typów} danych\add{,}
\item \change{Z}{z}apewnienie dobrej wydajności podczas wyszukiwania połączeń lotniczych\add{.}
\end{itemize}
Części te zostaną opisane w podrozdziałach bieżącego rozdziału.

\section{Źródła danych o połączeniach lotniczych}
Największą trudnością podczas pisania pracy było znalezienie odpowiednich zasobów danych\add{,} które\delete{,} nie byłyby płatne oraz \delete{które} zapewniałyby rzetelne i sprawdzone dane lotnicze. Poszukiwania zaczęto od złożenia podań do \change{centr}{centrów} danych o dostęp do ich zasobów. Większość z nich \change{wymagała}{wymaga} opłaty za swoje usługi\add{,} które \change{sięgały}{sięgają} nawet 10 000\$. Niektóre z nich \change{oferowały}{oferują} jednak darmowy\add{, choć limitowany,} dostęp do ich zasobów\delete{, jednak był to dostęp limitowany}. Na potrzeby pracy wybrano serwis FlightLookup jako głównego dostawcę danych. Informacje przez niego \change{dostarczone stanowiły lwią }{dostarczane stanowią większą} część odpowiedzi serwera. Darmowy dostęp jest \change{limitowany 500 zapytaniami}{do 500 zapytań} w trakcie miesiąca.
Dodatkowymi źródłami danych \change{były}{są}:
\begin{itemize}[noitemsep,topsep=0pt]
\item Skyscanner - serwis \change{udostępniające}{udostępniający} średnie ceny przelotów w określonym przedziale czasowym oraz informacje dotyczące dozwolonego bagażu\add{,}
\item Aviation Edge - usługa chmurowa udostępniając\change{e}{a} dane o liniach lotniczych\add{,} 
\item ourairports.com - strona internetowa umożliwiając\change{e}{a} pobranie danych o większości lotnisk na świecie\add{.}
\end{itemize}
Skyscanner jest komercyjną aplikacją zbierającą wiele rodzajów danych lotniczych. Jej \change{szczegółowa funkcjonalność zostanie opisana}{szczegółowe działanie zostanie opisane} w następnym rozdziale. W tej części pracy zostanie \change{opisane}{przedstawione} użycie zasobów tego produktu \add{w~stworzonym oprogramowaniu}. Zasoby Skyscanner'a dostarczyły danych \delete{o aktualnych cenach wyszukiwanych lotów oraz danych} dotyczących dozwolonego bagażu podczas podróży. \change{Ceny}{Aktualne ceny} lotów zostały pobrane z serwisu RapidApi korzystającego wewnętrznie z zasobów aplikacji Skyscanner. \change{Znajduje się on}{Jest on dostępny} pod adresem sieciowym: \url{https://rapidapi.com/skyscanner/api/skyscanner-flight-search}.

Użycie jego \change{serwisów}{usług} \change{wymagało}{wymaga} podania miejsca jak i daty wylotu oraz przylotu. Dodatkowym \change{wymaganym}{obowiązkowym} parametrem \change{był}{jest} unikalny kod walutowy który umożliwiał zwrócenie poprawnych wyników. Oprócz cen przelotów Skyscanner posiada też stronę internetową z tabelą opisującą dozwolone wymiary oraz wagę bagażu podczas przelotu. Adres tej strony to:\\ \url{www.skyscanner.net/news/tips/check-in-luggage-size-and-weight-restrictions}\\
Zawartość tej strony jest parsowana przez część serwerową\add{,} jednak to zagadnienie zostanie opisane \change{szerzej}{bardziej szczegółowo} w następnym podrozdziale.

Kolejnym ważnym źródłem danych jest usługa chmurowa Aviation Edge. Serwis ten \change{w przejrzysty sposób} udostępnia \add{czytelne} dane o liniach lotniczych wykorzystując do tego interfejs REST\footnote{Representational \change{s}{S}tate \change{t}{T}ransfer}. Do użycia tej usługi wymagane \change{było}{jest} podanie unikalnego kodu linii lotniczej w celu jej zidentyfikowania oraz pobrania danych w postaci JSON. Aviation Edge jest bezpłatnym serwisem\change{, d}{. D}o korzystania z jego zasobów wymagane jest tylko założenie konta w celu uzyskania klucza identyfikującego użytkownika.

Ostatnim źródłem danych jest strona internetowa ourairport.com znajdująca się pod adresem internetowm: \url{http://ourairports.com/}. Jej zasobem, który został wykorzystany jako źródło danych jest plik \change{csv}{CSV} zawierający ponad 50000 rekordów \add{dotyczących} lotnisk. Przechowuje \add{on} dane z szerokiego \change{przekroju}{spektrum} typów lotnisk począwszy od tych małych dedykowanych dla awionetek\add{,} aż po największe lotniska świata takie jak London Heathrow Airport. Dane te \change{zabrane są w całości}{gromadzone są} w \delete{postaci} pliku CSV\add{,} który jest pobierany przez część serwerową. \change{Poddawane są procesowi filtracji}{Następnie informacje te są filtrowane} w celu wyeliminowania lotnisk obsługujących poniżej 4 tysięcy pasażerów rocznie.

\section{Parsowanie danych}
\hint{Zmieniłem tytuł rozdziału.}
Dane dostarczone przez zewnętrzne \change{serwisy}{usługi} \change{prezentowały swoją treść}{są} w różnych formatach. Aby zebrać pełną odpowiedź serwera \change{należało}{należy} \delete{w pierwszym kroku} sparsować pojedyncze elementy a następnie zbudować z nich obiekt\add{y} języka Java.
...
XML jest językiem znaczników, nie jest on językiem programowania, magazynem danych czy też protokołem transportu sieciowego. XML oferuje możliwość formatowania danych\change{,}{.} \change{z}{Z}apewnia to \change{ich prawdziwą wieloplatformowość}{im dostępność na wielu platformach} i odporność na upływ czasu. \change{Dotychczas}{W~starszych wersjach standardu XML} dokument zapisany za pomocą dowolnego oprogramowania na jednej platformie nie dawał się odczytywać na innej. \change{Najnowsza w}{W}ersja 3.2 \change{zmieniła tą właściwość}{usunęła tę wadę}. Zapewnia ona obsługę Unicode, co oznacza, że dokumenty XML składają się w całości ze znaków \change{pobranych z}{należących do} zestawu \delete{znaków} Unicode. 

Wyszukiwanie informacji o połączeniach lotniczych rozpoczynało się od odebrania danych z serwisu FlightLookup w postaci XML. Jest to najważniejsza operacja podczas \delete{procesu} wyszukiwania połączeń lotniczych. Dane zebrane \change{w tej części}{na tym etapie} są parametrami wyszukiwania podczas korzystania ze źródeł danych wykorzystanych w \change{późniejszym etapie}{późniejszej fazie} tego procesu. Przykład odebranej treści znajduje się na listingu \change{1.2.1}{\ref{list::xml}}\delete{ załączonym na poprzedniej stronie}. \hint{Skąd jest Pan pewien, że ten listing będzie na poprzedniej stronie? Być~może w~ostatecznej wersji znajdzie się na następnej. \LaTeX posiada system odwołań do listingów, tabel, rozdziałów, rysunków itp., który trzeba zastosować. Wymaga on użycia poleceń \texttt{label} i~\texttt{ref}, Przykład dla listingów ma Pan w tym akapicie. Resztę proszę uzupełnić samodzielnie.}

Operacja parsowania obiektów XML na obiekty języka Java zostanie \change{szerzej}{dokładniej} opisana w rozdziale o implementacji. Załączony listing \delete{1.2.1} przedstawia jeden z elementów danych XML o nazwie \textit{FlightDetails}. Zawiera on przykładowe pola takie jak: \textit{TotalMiles} \change{czy}{i} \textit{FLSFlightDays}. Elementy te zostały odwzorowane w postaci klas języka Java o takich samych nazwach jak nazwa elementu. Powstała klasa posiada też takie same pola jak obiekt w XML.

\subsection{Notacja JSON}
Notacja JSON\footnote{JavaScript Object Notation} jest \change{modernistycznym}{nowoczesnym} sposobem prezentacji danych. \hint{Zanim użyje Pan jakiegoś ,,mądrego" wyrazu proszę sprawdzić w~słowniku jego znaczenie.} Wywodzi się ona z języka JavaScript\change{ gdzie}{, w~którym} została głównym formatem prezentacji obiektów tej technologii. Notacja \change{Json}{JSON} jest zbudowana na dwóch strukturach:
\begin{itemize}[noitemsep,topsep=0pt]
\item \change{K}{k}olekcji par nazwa/wartość, w zależności od języka programowania zrealizowana jako obiekt, słownik bądź kolekcja danych\add{,}
\item \change{Posortowana lista}{uporządkowanej listy} wartości\change{. W}{, w~}większości języków zrealizowan\change{a}{ej} jako tablica, wektor, lista lub sekwencja\add{.}
\end{itemize}
Są to uniwersalne struktury danych. Prawie wszystkie nowoczesne języki programowania wspierają je w odpowiedniej dla siebie formie. Notacja JSON jest to format danych, który jest wymienny \change{z}{między} językami programowania. Ta właściwość czyni ją najpopularniejszym formatem wymiany informacji między aplikacjami oraz mikroserwisami\cite{json}. W stworzonej aplikacji dane w tym formacie dotyczyły \change{liniach}{linii} lotniczych oraz cen przelotów. W pierwszym przypadku obiekt JSON można było w prosty sposób skonwertować na obiekt języka Java. Przykładową strukturę zaprezentowano na listingu \delete{1.2.3}\\
Kod źródłowy odpowiedzialny za konwersję tego obiektu zostanie przedstawiony w rozdziale \delete{piątym}.
...
HTML jest standardowym językiem znaczników \change{dla}{do} tworzenia stron \change{internetowych}{WWW}. Dokumenty HTML są podstawową treścią jaką generują przeglądarki internetowe. W pracy dyplomowej źródłem danych o wymiarach i wadze dozwolonych bagaży była strona internetowa aplikacji Skyscanner\cite{html}. \change{Poniżej przedstawiono}{Listing \ref{lst::html} przedstawia} fragment \add{takich} danych zapisanych w formacie HTML\delete{ które posłużyły do celów pracy}.
...
Wyszukiwanie tak złożonych danych jak informacje o połączeniach lotniczych, a następnie parsowanie ich\add{,} niesie za sobą pewne konsekwencje. Są \change{to konsekwencje czasowe,}{one związane z~czasem.} \change{u}{U}żytkownik powinien otrzymać interesującą go treść w~\delete{czasie}jak najkrótszym \add{czasie}. W celu \change{optymalizacji}{poprawy} wydajności aplikacji wprowadzono mechanizmy skracające czas odpowiedzi części serwerowej.
Dla zbierania danych dotyczących lotnisk oraz bagażów zaimplementowano rozwiązania polegające na pobieraniu pełnych \change{zasobów tych danych}{zbiorów tych informacji} do bazy danych lub do pliku tekstowego znajdującego się na serwerze. Pozwoliło to na \change{pominięcie opóźnienia sieciowego związanego z potencjalną koniecznością pobierania informacji}{ograniczenie częstości pobierania danych} ze stron lub zewnętrznych baz danych.

Kolejnym rozwiązaniem było wprowadzenie funkcyjnego stylu programowania w kluczowych elementach części serwerowej\add{,} które odpowiadały za wyszukiwanie połączeń lotniczych.  Programowanie funkcyjne wprowadzone w Javie 8 pozwala skrócić \change{operacje}{czas trwania niektórych operacji wykonywanych} \change{po stronie wirtualnej maszyny}{przez maszynę wirtualną} Javy, a więc też \change{zaoszczędzić cenne milisekundy w trakcie}{czas} wyszukiwania lotów.
Implementacje tych rozwiązań \change{znajdują się}{są opisane} w rozdziale czwartym.

Ostatnim \add{zastosowanym} mechanizmem \change{jest rozwiązanie cachowania}{są pamięci podręczne (ang. \emph{cache})} danych. \delete{Caching to mechanizm przyspieszający działanie aplikacji oraz zwiększającym jej ogólną wydajność.} \hint{Pamięci podręczne niczego nie przyspieszają, skracają czas dostępu do danych. Poza tym już to Pan pisał.} Część danych, która jest gromadzona \change{w aplikacji}{przez aplikację w~zasobach} o długich czasach dostępu i \change{niższej}{względnie niskiej} przepustowości jest dodatkowo przechowywana w pamięci RAM o lepszych parametrach\cite{ehcache}. \hint{Mam nadzieje, że dobrze zrozumiałem sens poprzednich zdań. Jeśli nie, to proszę poprawić.} \change{Jes}{To rozwiązanie jest} \change{podstawą wszystkich nowoczesnych systemów}{często stosowane w~nowoczesnych systemach} informatycznych. Odczytywanie danych z tej pamięci jest \change{najszybsze porównując go}{szybsze w porównaniu} na przykład z odczytywaniem \delete{z} dokumentów XML\change{,}{ i~}JSON \add{z~pamięci masowej}, \add{zewnętrznej} bazy danych czy też zasobów sieciowych. \change{Jego przeznaczeniem}{Przeznaczeniem pamięci masowych} jest skracanie czasu odpowiedzi dla określonych zapytań do części serwerowej, które są zwielokrotnione i \change{wywoływane}{wykonywane} przez wielu użytkowników. Wykonując operacje wyszukiwania lotów, moduł wyszukiwania sprawdza czy w\change{ cache'u}{~pamięci podręcznej} znajdują się poszukiwanie \change{operacje}{wyniki}. Jeśli tak, zwraca je użytkownikowi. W przeciwnym wypadku  wyszukuje loty standardowym sposobem a wynik zapisuje do \change{cache'u}{pamięci podręcznej}. Dla danych o liniach lotniczych, bagażach oraz dla całego obiektu lotu w części serwerowej stworzono 3 kontenery \change{cache'u}{pamięci podręcznych}. Czas przetrzymywani\change{e}{a} danych w tych kontenerach wynosi odpowiednio: 7 dni, 3 dni oraz 3 godziny. \hint{Dlaczego akurat takie wartości? Czym jest to uzasadnione? Czy na pewno są tak dobrane, że użytkownik nie będzie otrzymywał nieakualnych danych? Proszę napisać.} \change{Przy wyborze informacji które powinny znaleźć się w pamięci podręcznej sugerowano się temperaturą danych.}{Wybór informacji, które powinny znaleźć się w pamięci podręcznej bazuje na tzw. temperaturze danych.} Trzy wcześniej wymienione zasoby oceniono z największym prawdopodobieństwem na ich użycie przez użytkowników aplikacji. \hint{Proszę poprawić ostatnie zdanie, bo nie bardzo wiadomo, o~co w~im chodzi.}
Jednostki te wybrano uwzględniając wrażliwość tych danych na ciągłe zmiany w systemach lotniczych. Implementacja tych mechanizmów \change{wraz z kodem źródłowym znajduje się w}{została opisana w~}rozdziale czwartym.
%\newpage

\chapter{Przegląd istniejących rozwiązań}
Analiza istniejących rozwiązań, \add{czyli} aplikacji wyszukujących połączenia lotnicze pozwoliła \change{nadać pracy bardziej precyzyjne wymagania}{sprecyzować wymagania względem budowanego oprogramowania}\delete{ oraz zaprojektować jej ogólny przebieg}. W \change{i}{I}nternecie można znaleźć wiele aplikacji o\change{ podobnych lub takich samych funkcjonalnościach}{podobnej lub takiej samej dziedzinie zastosowania,} jak opracowana aplikacja. W tym rozdziale \delete{zostaną} opisane \add{są} najbardziej znane wyszukiwarki lotów dostępnych na rynku. 

\section{Wyszukiwarka lotów Skyscanner}
Pierwszym przykładem \change{została}{jest} aplikacja internetowa Skyscanner. \change{Jest to}{To} wyszukiwarka lotów, która umożliwia użytkownikom szukanie \delete{lotów} według ceny i lokalizacji. Oprócz \delete{funkcji} wyszukiwania lotów, Skyscanner oferuje opcje \change{wyszukiwania hotelów}{szukania hoteli} blisko lotnisk oraz \change{wypożyczenia}{wypożyczania} auta w pobliżu lotniska docelowego. \delete{Aplikacja ta została stworzona oraz wdrożona w 2002 roku. Od tego czasu firma Ctrip, która jest właścicielem tego produktu zatrudnia ponad 200 pracowników.} \hint{Ta informacja nic nie wnosi do zagadnień związanych z~tematem pracy.} Warto wspomnieć o\add{~innej} jej \delete{innej} usłudze\change{ która udostępnia dane}{, czyli udostępnianiu danych~}o połączeniach \change{lotniczym}{lotniczych} zewnętrznym firmom i \change{deweloperom}{programistom.} \hint{W~języku polskim ,,deweloper" to człowiek, który prowadzi firmę budującą domy, więc nie ten wydział.} \change{Jej dane były brane pod uwagę w czasie szukania źródeł danych lecz Skyscanner}{W~związku z~tym aplikacja Skyscanner była brana pod uwagę, jako potencjalne źródło danych, ale jej właściciel} wymaga dużych opłat za swoje usługi, w warunkach akademickich niemożliwe było \delete{z ich} skorzystanie \add{z~nich}.
Aplikacja ta jest dostępna w ponad 30 językach oraz używana przez 60 milionów użytkowników miesięcznie. \delete{Aplikacja wiele razy nagradzana była za swoją funkcjonalność i użyteczność użytkownikom.} Skyscanner znajduje się pod adresem sieciowym: 
\url{https://www.skyscanner.net/}.

Opisywana aplikacja \change{ma dla swoich użytkowników szereg przydatnych funkcjonalności.}{oferuje swoim użytkownikom szereg przydatnych funkcji.} Korzystające z jej osoby mogą \change{korzystać z kompleksowego modułu}{posługiwać się kompleksowym modułem} wyszukiwania połączeń lotniczych. W panelu wyboru parametrów lotów można \change{wybrać}{określić} lotnisko wylotu, lotnisko docelowe jak i daty \change{wylotu}{lotu}. Skyscanner oferuje \change{3}{trzy} sposoby wyszukiwania lotów: lot powrotny, lot w jedną stronę oraz podróż wieloetapowa. Dodatkowo użytkownik jest w stanie sprecyzować liczbę osób podczas podróży jak i klasę biletu lotniczego. Ciekawą funkcjonalnością jest możliwość dodania pobliskich lotnisk do tych wybranych przez użytkownika. Znajduje ona zastosowanie, gdy na przykład aplikacja nie może znaleźć lotów z wybranego lotniska. Następuje wtedy wyszukiwanie lotów z pobliskich lotnisk o zadanym promieniu odległości.
