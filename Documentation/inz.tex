...
Opisywana aplikacja \change{ma dla swoich użytkowników szereg przydatnych funkcjonalności.}{oferuje swoim użytkownikom szereg przydatnych funkcji.} Korzystające z jej osoby mogą \change{korzystać z kompleksowego modułu}{posługiwać się kompleksowym modułem} wyszukiwania połączeń lotniczych. W panelu wyboru parametrów lotów można \change{wybrać}{określić} lotnisko wylotu, lotnisko docelowe jak i daty \change{wylotu}{lotu}. Skyscanner oferuje \change{3}{trzy} \change{sposoby}{opcje} wyszukiwania lotów: \change{lot powrotny}{z~lotem powrotnym}, lot w jedną stronę oraz podróż wieloetapowa. Dodatkowo użytkownik \change{jest w stanie sprecyzować}{może określić} liczbę osób \change{podczas}{biorących udział w~}podróży jak i klasę biletu lotniczego. \change{Ciekawą funkcjonalnością}{Interesującą funkcją} jest możliwość dodania pobliskich lotnisk do tych wybranych przez użytkownika. Znajduje ona zastosowanie, gdy na przykład aplikacja nie może znaleźć lotów z wybranego lotniska. Następuje wtedy wyszukiwanie lotów z pobliskich lotnisk \change{o zadanym promieniu odległości}{znajdujących się w~zasięgu o~kształcie koła o~zadanej długości promienia}.
Oprócz \change{bogatego}{rozbudowanego} interfejsu do wyboru parametrów lotów Skyscanner oferuje bardzo dobrą wydajność podczas wyszukiwania. Loty bezpośrednie wyszukiwane są natychmiastowo. Na loty z zaznaczoną opcj\change{a}{ą} możliwych przesiadek trzeba poczekać \add{zwykle} kilka sekund. Interfejs aplikacji Skyscanner \change{zostanie}{jest} zaprezentowany \change{na następnej stronie}{na ilustracji \ref{fig:skyscanner_main}}.
%\newpage
\begin{figure}[!ht]
\centering
...
\end{figure}
Po wybraniu odpowiednich parametrów \add{w~widocznym na rysunku \ref{fig:skyscanner_main} menu} należy nacisnąć zielony przycisk po prawej\add{,} aby rozpocząć proces wyszukiwania lotów. \change{Jego przykładowe}{Przykładowe} wyniki zaprezentowano na rysunku \change{2.1.2}{\ref{fig:skyscanner_result}}\change{, m}{. M}ożna na nim zauważyć wyszukane loty z dokładnymi informacjami o linii lotniczej obsługującej lot, miejsca wylotu oraz przylotu, czasie podróży jak i liczbie przesiadek. Z lewej strony dostępny jest panel dzięki któremu można dokonać 
\change{różnego rodzaju filtracji na otrzymanych wynikach}{filtrowania otrzymanych wyników}.
\begin{figure}[!ht]
\centering
...
Warto zwrócić uwagę na panel podróży wieloetapowej, który jest dostępny po kliknięciu  odpowiedniego przycisku powyżej pola \delete{do} wyboru lotniska wylotowego. Można w nim wybrać do siedmiu osobnych połączeń lotniczych i na ich podstawie zaplanować \delete{swoją} podróż. Każde z tych połączeń jest domyślnie \change{połączeniem}{lotem} w jedną stronę. 
\begin{figure}[!ht]
\centering
...
Kolejnym przedstawianym rozwiązaniem jest aplikacja internetowa Google Flights firmy Google. Jest to jeden z głównych internetowych produktów \change{giganta}{firmy} z Kalifornii\change{. W pełni integruje się}{, zintegrowany} z innymi usługami tej firmy\add{,} co czyni go bardzo praktycznym systemem. Google Flights zostało wdrożone 13 października 2011 roku. Ma zbliżone \change{funkcjonalności}{funkcje} do wcześniej opisywanego konkurenta\add{,} Skyscanner. Oferuje jednak pewne \change{innowacje}{dodatkowe udogodnienia}. Pozwala \add{on} użytkownikowi na wyszukiwanie lotów \change{określając okres czasowy go interesujący oraz budżet na jaki pozwala jego portfel}{według takich kryteriów, jak czas trwania podróży oraz koszt}. \hint{Nie jestem pewien, czy chodziło Panu o czas trwania podróży. Jeśli nie, to proszę poprawić, ale na pewno nie ma czegoś takiego jak ,,okres czasowy". Okres to czas i~tyle.} Największą zaletą tej aplikacji jest jej \change{szybkość}{efektywność}. \change{Dzięki spokrewnieniu z silnikiem Google Search szybkość wyszukiwania połączeń lotniczych jest bezkonkurencyjna na rynku wyszukiwarek lotów.}{Silnik tej wyszukiwarki jest podobny do tego stosowanego w~Google Search.} \change{Szybkość ta została uzyskana}{Wydajność tego oprogramowania została uzyskana} przez użycie wyspecjalizowanych algorytmów szukających firmy Google oraz specjalnemu indeksowaniu danych. Oprócz \change{samej funkcjonalności}{tej zalety} aplikacja prezentuje \add{również} czytelny i \change{responsywny interfejs}{dostosowujący się do używanego medium (ang. \emph{responsive}) interfejs}, który \change{zyskał aprobatę ze strony}{jest dobrze oceniany przez} użytkowników.
...
\end{figure}
\change{Na rysunku 2.2.1 przedstawiono}{Rysunek \ref{fig:google_flights_search_panel} przedstawia} panel wyszukiwania lotów oferowany przez opisywaną aplikację. Dostępny jest też komponent podróży wieloetapowej który znalazł się też w poprzednio opisywanej aplikacji oraz w \change{części praktycznej}{w~aplikacji powstałej w~ramach} tej pracy.
...
\end{figure}
Wszystkie produkty firmy Google, które \change{można obejrzeć w przeglądarce internetowej}{dostępne są za pomocą przeglądarki WWW} charakteryzują się specyficznym stylem interfejsu. Jest to \change{platforma}{styl} Material Design, \change{która wyznacza}{który określa} specyfikację pojedynczych elementów aplikacji Google'a. Rysunek \change{2.2.3}{\ref{fig:skyscanner_result}} przedstawia przykładowe wyniki wyszukiwania lotów. \change{Wyszukane loty znajdują się w}{Znajdują się one na } rozwijanej liście. Po kliknięciu w wybrany element listy rozwija się panel z dokładniejszymi informacjami o locie. Każdy element zaprezentowany na \change{poniższym}{wspomnianym} rysunku jest zgodny z~\delete{wcześniej wspomnianą platformą }Material Design. Widać na przykład zaokrąglenia przycisków charakterystyczne dla produktów marki Google. 
...
Obie opisane aplikacje są rozwiązaniami komercyjnymi, stworzonymi przez wieloosobowe zespoły programistów. Są \add{także} systemami sprawnie działającymi, z czytelnym \delete{oraz zrozumiałym} interfejsem graficznym. \change{Swoją funkcjonalnością są do siebie identyczne}{Dostarczają one niemal identycznych usług}\change{,}{ {\dywiz}} posiadają \change{tą}{tę} samą ofertę dla użytkownika\add{,} jednak różnią się jakością jej wykonania. 

\change{Zaletami}{Zaletą} aplikacji Skyscanner jest z pewnością rzetelność jej danych\change{. Jest to potwierdzone ze strony linii lotniczych, które wielokrotnie ją nagradzały}{, która jest potwierdzona licznymi nagrodami od linii lotniczych}. Warto też wspomnieć o jej systemie wyszukiwania lotów. \change{Ilość}{Liczba} wyszukanych lotów z przesiadkami często przekracza \change{100}{sto} wystąpień. Świadczy to o zaawansowanym silniku wyszukującym i analizującym różne przypadki połączeń. Za wady można uznać dosyć prosty i \change{nieresponsywny}{niedostosowany do potrzeb urządzeń mobilnych} interfejs użytkownika. Nie reprezentuje on poziomu nowoczesnych aplikacji. 

Druga wyszukiwarka jest \delete{bardzo} dopracowanym produktem \change{światowego giganta}{firmy Google}. \change{Jak wcześniej już wspomniano jego szybkość wyszukiwania danych jest bezkonkurencyjna}{Wyszukuje ona loty bardziej efektywnie niż rozwiązania konkurencji, między innymi Skyscanner}. \delete{Aplikacja Skyscanner znacznie ustępuje jej na tym polu.} Google Flights \change{jest sprzężona}{współpracuje} z systemami sprzedażowymi większości przewoźników lotniczych. Daje to możliwość kupna biletu bezpośrednio z jej \change{poziomu}{strony WWW} bez konieczności przechodzenia na zewnętrzną stronę linii lotniczej. \change{Jej dużą zaletą}{Zaletą wyszukiwarki} z pewnością jest dopracowany interfejs zgodny ze standardami \change{reprezentowanymi w rodzimych produktach}{firmy Google}. Gwarantuje on \change{pełną responsywność}{dostosowanie do używanego medium} oraz czytelność prezentowanych danych. Wyszukiwarka Google Flights \change{prezentuje się lepiej}{wydaje się być lepszym rozwiązaniem} od aplikacji Skyscanner jednak różnica \add{jakości} je dzieląca nie jest duża.

\delete{W opracowywanej aplikacji starano się w rzetelny sposób zrealizować podstawowe \change{funkcjonalności}{funkcje} obu opisywanych wcześniej \change{aplikacji}{wyszukiwarek}. Komercyjne aplikacje mają dużą przewagę biorąc pod uwagę szybkość wyszukiwania. W powstałej aplikacji czas odpowiedzi na żądania użytkownika jest zauważalnie \change{większy}{dłuższy}. W warunkach akademickich trudno było o uzyskanie konkurencyjnych rezultatów.}  \hint{Ten wykreślony akapit proszę przenieść do podsumowania pracy. Tutaj powinny się pojawić wymagania odnośnie tworzonej przez Pana aplikacji, które wynikają z~analizy istniejących rozwiązań.}
%\newpage
\hint{Proszę wyrzucić z pracy wszystkie polecenia \texttt{newpage}.}

\chapter{Projekt aplikacji}
\delete{Następnym krokiem po analizie zagadnienia oraz zaznajomieniem się z przykładami istniejących rozwiązań było opracowanie projektu aplikacji. Zadanie to jest mocno związane z dziedziną architektury oprogramowania. Dobrze zaprojektowany system pozwala na uniknięcie wielu kosztownych problemów w przyszłości. Projektowanie aplikacji wymusza na osobie to wykonującej dostatecznie wczesne rozważenia najważniejszych aspektów projektowania w odniesieniu do całego tworzonego systemu\cite{architektura}. Wynikiem zrealizowania zadań analitycznych są wymagania postawione tworzonemu systemowi. Wynikiem tych działań mogą być takie elementy jak model dziedziny, model organizacyjny czy model wymagań . Projekt aplikacji ukierunkowuje zadania implementacyjne razem z projektowaniem szczegółowym, implementacją kodu, inegracją modułów oraz testowaniem.} \hint{Wstęp ma opisywać zawartość rodziału, a~nie zaś być rozważaniami na temat projektowania aplikacji. Proszę to poprawić, zaczynając od fragmentu, który dalej zamieszczę.}
\add{W~tym rozdziale jest opisany projekt aplikacji. \dots}
\hint{Dlaczego zaczyna Pan od architektury, skoro nie wiadomo jakie są wymagania odnośnie projektowanego oprogramowania? Należy rozpocząć od podrodziału, gdzie będą one wymienione. Dobrze by było gdyby pojawił się w~nim diagram przypadków użycia.}

\section{Architektura aplikacji}
\hint{Zmieniłem tytuł podrozdziału. Architektura jest już ogólnym przedstawieniem systemu. Nie może być bardziej ogólna.}
Projektowanie aplikacji \change{wymagało}{rozpoczęto od} stworzenia \delete{czytelnego} schematu obrazującego poszczególne części systemu oraz \change{sposoby}{schematy} komunikacji między nimi.
Aplikację można podzielić na \change{3 logiczne}{trzy} części: 
\begin{itemize}[noitemsep,topsep=0pt]
\item \change{C}{c}zęść serwerowa\add{,}
\item \change{C}{c}zęść kliencka\add{,}
\item \change{B}{b}aza danych\add{.}
\end{itemize}
\hint{Wydzielanie bazy danych jest dosyć dyskusyjne, bo można ją zaliczyć do części serwerowej. Proszę uzasadnić, że jest to konieczne.}
...
\change{Kluczowym zagadnieniem jest komunikacja między powyższymi komponentami. W stworzonej aplikacji istnieje kilka rodzajów transportu danych między modułami.}{Komunikacja między opisanymi kluczowymi elementami aplikacji odbywa się na kilka sposobów.} Każdy z nich \change{zostanie}{jest} szczegółowo opisany w dalszej części tego rozdziału.
...
\end{figure}
\hint{Projekt powinien być niezależny od technologii, aby nie narzucać sposobu implementacji. W~związku z~tym mam wątpliwości co do tego rysunku. JDBC jest technologią związaną z językiem Java, SQL dosyć konkretnie wskazuje, że baza danych jest relacyjna. Na pewno trzeba poprawić ,,Lokalny cache w~pamięci RAM". Ten opis powinien być następujący ,,Lokalna pamięć podręczna". Dodatkowo nie ,,część serwerowa", a~,,serwer".}
%\newpage
\change{Na rysunku 3.1.1zamieszczony został}{Rysunek \ref{fig:architecure_diagram} zawiera} schemat architektury \change{stworzonego systemu}{opracowanej aplikacji}. Przedstawia on \change{rozdział strukturalny}{strukturę} aplikacji oraz \change{drogę jaką musi odbyć żądanie użytkownika w celu uzyskania pożądanej odpowiedzi}{ścieżkę przetwarzania żądania pochodzącego z~przeglądarki WWW}. Użytkownik po wypełnieniu odpowiednich formularzy w części klienckiej rozpoczyna proces wyszukiwania połączeń lotniczych. Oprogramowanie klienta po pobraniu od użytkownika niezbędnych parametrów takich jak miejsce wylotu, \change{czy}{lub} rodzaj lotu zwraca się do części serwerowej o wyszukanie lotów\delete{ zgodnych z podanymi parametrami}. Komunikacja między tymi modułami odbywa się \change{protokołem}{z~użyciem protokołu} HTTP. \change{Po wywołaniu, część serwerowa zwraca się do modułu wyszukiwania lotów}{Odebrane żądanie część serwerowa przekazuje do modułu wyszukiwania lotów}. Moduł ten sprawdza wszystkie źródła danych zamieszczone na schemacie \delete{3.1.1}. \hint{Proszę tu umieścić referencję do rysunku.}  Zebrane \change{dane }{informacje} są \delete{odpowiednio} parsowane i przekazywane modułowi wyszukiwania który z kolei zwraca je kontrolerowi wywołanemu przez użytkownika. \hint{?? Moduł wyszukiwania zwraca przekazuje dane do modułu wyszukiwania? Proszę poprawić.} Kontroler następnie przekazuje dane do części klienckiej, która wyświetla je w  graficznym interfejsie użytkownika. 

\section{Diagram UML}
\hint{Co to za tytuł??? Pomijając już to, że jest około 14 takich diagramów, to UML jest narzędziem, które używane jest do projektowania, a~nie elementem aplikacji!! Proszę to poprawić.} 
\delete{Kluczowym elementem podczas tworzenia oprogramowania jest komunikacja. Przełożenie złożonej logiki biznesowej na język informatyczny to zadanie trudne oraz wymagające zrozumienia wymagań potencjalnego odbiorcy tworzonej dokumentacji technicznej. Ważny jest więc czytelny sposób opisu oprogramowania, który byłby w stanie zrozumiale przedstawić swoja treść wszystkim osobom się z nim zaznajamiającym\cite{uml}. Podstawowym narzędziem w tej dziedzinie stał się język UML}\footnote{Unified Modelling Language}. \hint{To nie jest praca na temat UML!! Proszę się skupić na opisywaniu budowy aplikacji, a nie na opisie narzędzi do tego służących.}

\begin{figure}[!ht]
\centering
\includegraphics[scale=0.56, keepaspectratio]{activity_diagram.png}
\caption{Diagram aktywności \change{procesy}{dla procesu} wyboru parametrów wyszukiwania}
\label{fig:activity_diagram}
\end{figure}
Dla opracowanej aplikacji przygotowano diagram aktywności widoczny na rysunku \delete{3.2.1} \hint{Jak wcześniej.} Obrazuje on przebieg \change{wydarzeń}{czynności,} które wykonywane są przez użytkownika \hint{To jest diagram dla użytkownika, czy dla oprogramowania? Proszę poprawić.} po wczytaniu przez niego strony startowej. \delete{Jego najważniejszą funkcją jest ukazanie sekwencji kroków wykonywanych w części klienckiej.} 
%\newpage
...
\section{Baza danych}
\delete{Baza danych nie była sprecyzowana w wymaganiach pracy dyplomowej.} \hint{Nie było także sprecyzowane, że ma Pan użyć komputera, a~jednak Pan to zrobił.} \change{Została ona użyta opcjonalnie}{Baza danych jest używana w~aplikacji} jako miejsce dające szybki dostęp do danych o lotniskach. \hint{To do czego użył Pan pamięci podręcznej?} \delete{Użyto darmowego rozwiązania bazodanowego MySQL. Jest to system zarządzania relacyjnymi bazami danych rozwijany w przeszłości przez firmę MySQL AB, a obecnie przez korporację Oracle. Stanowi szybko działające oprogramowanie i obecnie jest jednym z popularniejszych serwerów baz danych dostępny na licencji GPL}\footnote{General Public License} jak i w wersjach komercyjnych \cite{mysql}. \hint{To są informacje potrzebne w~rozdziale o~implementacji. Teraz opisuje Pan projekt.}
...
\label{fig:database}
\end{figure}
\delete{Jedyna tabela znajdująca się w bazie danych to tabela lotnisk z parametrami wskazanymi powyżej.} \hint{Proszę opisać ten rysunek, a~nie tylko napisać, że parametry ,,widać". Poza tym proszę używać odnośników do rysunków. Skąd pewność, że ten tekst znajdzie się ostatecznie pod rysunkiem?}
...
Rozwiązanie problemu wyszukiwania połączeń lotniczych \change{było}{jest} zadaniem \change{trudnym i skomplikowanym}{złożonym}. \change{Wymagało}{Wymaga} \delete{to} poradzenia sobie z problemem przeszukiwania wielu źródeł danych oraz konwertowania informacji przez nie zwracany\add{ch} \change{do formy zrozumiałem dla części serwerowej oraz klienckiej}{do postaci, w~której mogą być przetwarzane zarówno przez stronę kliencką, jak i~serwerową}. \change{Zagadnienie to wymagało stworzenia wyodrębnionego modułu wyszukiwania połączeń lotniczych który został osadzony jako część oprogramowania serwerowego powstałej aplikacji.}{W~związku z~tym wyodrębniono w~strukturze aplikacji moduł, będący elementem części serwerowej oprogramowania, który jest odpowiedzialny za te czynności.}

\begin{figure}[!ht]
\centering
\includegraphics[scale=0.50, keepaspectratio]{search_module.png}
\caption{Schemat \change{architektoniczny}{architektury} modułu wyszukiwania lotów}
\label{fig:search_module}
\end{figure}
\hint{Ten schemat znowu budzi moje wątpliwości, bo zawiera informacje, o~konkretnych technologiach. Niech jednak zostanie, bo przerobienie go zabrałoby Pan zbyt dużo czasu, a~jest on dosyć istotny w~tym rozdziale.}

Na rysunku \delete{3.5.1} \hint{Pisałem wcześniej.} przedstawiono schemat \delete{zaprojektowanego} modułu wyszukiwania połączeń lotniczych. \delete{Zobrazowano na nim przebieg procesu wyszukiwania z uwzględnieniem źródeł danych, sposobów ich uzyskiwania oraz rodzajów odpowiedzi.} \hint{To w~końcu jest to schemat budowy modułu, czy przebiegu procesu? To są dwie różne rzeczy. Proszę poprawić.} \change{Górny opis strzałek oznacza metodę zwrócenia się o dane, natomiast dolny opisuje rodzaj zwróconej treści.}{Napisy nad strzałkami są nazwami protokołów służących do realizacji zapytań, a~te pod strzałkami oznaczają format treści zwróconej w~odpowiedzi.} Wyszukiwanie \change{rozpoczynało}{zaczyna} się od wysłania żądania z kontrolera serwerowego do \change{omawianego}{opisywanego} modułu. Żądanie to \change{musiało}{musi} uwzględniać \delete{w sobie} parametry wyszukiwania. Moduł swoją pracę rozpoczyna od pozyskania danych lotniczych z serwisu FlightLookup. Treść odpowiedzi stanowi większą część całego obiektu lotu który \change{ma zostać zwrócony}{jest zwracany}. Po otrzymaniu tych danych moduł zwraca się do lokalnej bazy danych o udostępnienie danych lotnisk \change{podając przy tym ich unikalne kody}{na podstawie ich unikalnych kodów} IATA\footnote{International Air Transport Association}. Następnym krokiem jest pozyskanie ceny lotu. Czynność ta rozpoczyna się od sprawdzenia\add{,} czy poszukiwana informacja \delete{nie} znajduje się w \change{cache'u}{pamięci podręcznej}. Jeśli takie dane są \change{zwrócone, cache}{obecne, to pamięć podręczna} \delete{natychmiastowo} zwraca odpowiedni zasób. Jeśli natomiast taka informacja nie jest dostępna, moduł zwraca się do \change{serwisów}{serwerów} firmy Skyscanner o udostępnienie cen \change{na}{za} poszukiwany przelot. \change{Analogiczna sytuacja dokonuje się w przypadku pozyskiwania}{Analogicznie przebiega pozyskiwanie} danych o linii lotniczej obsługującej lot. Ostatnim etapem \add{wyszukiwania} jest \change{wyszukanie}{znalezienie} informacji o dozwolonym bagażu podczas lotu. Jest to zrealizowane przez zwrócenie się do strony internetowej aplikacji Skyscanner posiadającej aktualne dane. Jej adres został wspomniany w rozdziale drugim. Po zebraniu wszystkich poszukiwanych informacji oraz sparsowaniu ich, obiekt lotu zwracany jest przez moduł wyszukiwania do kontrolera a następnie do części klienckiej gdzie użytkownik może obejrzeć wyniki całego procesu wyszukiwania.
